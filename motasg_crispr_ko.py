...
from MOTASG_Foundation.downstream import MOTASG_KO_Reg, DSGNNEncoder
...
def train_model(train_dataset_loader, current_cell_num, num_entity, name_embeddings, desc_embeddings, pretrain_model, model, device, optimizer, args):
    ...
    for batch_idx, data in enumerate(train_dataset_loader):
        ...
        batch_ptr = data.ptr
        # Separate knockout indices by batch
        batch_size = len(batch_ptr) - 1
        batch_ko_masks = []
        for b in range(batch_size):
            start_idx = batch_ptr[b]
            end_idx = batch_ptr[b+1]
            # Find knockout nodes for this specific batch (relative to batch start)
            batch_ko_indices = ko_mask[(ko_mask >= start_idx) & (ko_mask < end_idx)] - start_idx
            batch_ko_masks.append(batch_ko_indices)
            print(f"Batch {b}: {len(batch_ko_indices)} knockout nodes")
        ...
        output = model(x, pre_x, edge_index, internal_edge_index, ppi_edge_index, num_entity, name_embeddings, desc_embeddings, current_cell_num, ko_mask, batch_ko_masks)
...
def train(args, pretrain_model, device):
    ...
    e1, e2, e3, e4 = 2, 2, 3, 3  # Example values, adjust as needed
...
def arg_parse():
    ...
    parser.add_argument('--train_lr', type=float, default=0.1, help='Learning rate for training. (default: 0.0002)')
    parser.add_argument('--train_lr2', type=float, default=0.075, help='Learning rate for training. (default: 0.0001)')
    parser.add_argument('--train_lr3', type=float, default=0.05, help='Learning rate for training. (default: 0.000075)')
    parser.add_argument('--train_lr4', type=float, default=0.025, help='Learning rate for training. (default: 0.00005)')
    parser.add_argument('--train_lr5', type=float, default=0.01, help='Learning rate for training. (default: 0.00001)')
    ...
    parser.add_argument('--num_train_epoch', type=int, default=25, help='Number of training epochs. (default: 5)')
    parser.add_argument('--train_batch_size', type=int, default=2, help='Batch size for training. (default: 16)')
