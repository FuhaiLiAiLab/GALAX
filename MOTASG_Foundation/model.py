...
# custom modules
from MOTASG_Foundation.loss import info_nce_loss, ce_loss, log_rank_loss, hinge_auc_loss, auc_loss
...
class MOTASG_Foundation(nn.Module):
    def __init__(
        self,
        text_input_dim,
        omic_input_dim,
        ...
        self.cross_modal_fusion = nn.Linear(text_input_dim * 2 + omic_input_dim, input_dim)
    ...
    def train_step(self, data, num_entity, name_embeddings, desc_embeddings, optimizer, alpha=0.002,
        batch_size=2 ** 16, grad_norm=1.0
    ):
        self.train()

        num_node = num_entity * batch_size
        x, edge_index = data.x, data.edge_index # edge_index here is ppi_edge_index
